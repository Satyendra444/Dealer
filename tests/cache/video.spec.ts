...
import { VIDEO_TAGS } from '../../src/data/cache-tags';
import {
    warmCache,
...
test.describe('🎬 Video — Cache Invalidation', () => {
    for (const config of VIDEO_TAGS) {
        test.describe(config.label, () => {

            test(`Full cache lifecycle for tag="${config.tag}"`, async ({ request }) => {
                logger.banner(`VIDEO CACHE TEST — ${config.label}`);
                ...
                const spotChecks = getSpotCheckEndpoints('video');
                await verifyNoUnrelatedKeysDeleted(request, 'video', spotChecks);
                logger.pass('No unrelated keys affected');

                logger.banner('VIDEO CACHE TEST — PASSED ✅');
            });
