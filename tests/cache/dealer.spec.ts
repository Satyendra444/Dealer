...
import { DEALER_TAGS } from '../../src/data/cache-tags';
import {
    warmCache,
...
test.describe('🏪 Dealer — Cache Invalidation', () => {
    for (const config of DEALER_TAGS) {
        test.describe(config.label, () => {

            test(`Full cache lifecycle for tag="${config.tag}"`, async ({ request }) => {
                logger.banner(`DEALER CACHE TEST — ${config.label}`);
                ...
                const spotChecks = getSpotCheckEndpoints('dealer');
                await verifyNoUnrelatedKeysDeleted(request, 'dealer', spotChecks);
                logger.pass('No unrelated keys affected');

                logger.banner('DEALER CACHE TEST — PASSED ✅');
            });
