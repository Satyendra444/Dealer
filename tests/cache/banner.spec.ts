...
import { BANNER_TAGS } from '../../src/data/cache-tags';
import {
    warmCache,
...
test.describe('🖼️ Banner — Cache Invalidation', () => {
    for (const config of BANNER_TAGS) {
        test.describe(config.label, () => {

            test(`Full cache lifecycle for tag="${config.tag}"`, async ({ request }) => {
                logger.banner(`BANNER CACHE TEST — ${config.label}`);
                ...
                // ── STEP 4: Verify Redis keys deleted (indirect) ──
                logger.separator('STEP 4 — Verify Keys Deleted (Indirect)');
                logger.info('Re-calling API to confirm cache was cleared and rebuilt from source');
                ...
                const spotChecks = getSpotCheckEndpoints('banner');
                await verifyNoUnrelatedKeysDeleted(request, 'banner', spotChecks);
                logger.pass('No unrelated keys affected');

                logger.banner('BANNER CACHE TEST — PASSED ✅');
            });
