...
import { SEO_TAGS } from '../../src/data/cache-tags';
import {
    warmCache,
...
test.describe('🔍 SEO — Cache Invalidation (Very Important)', () => {
    for (const config of SEO_TAGS) {
        test.describe(config.label, () => {

            test(`Full cache lifecycle for tag="${config.tag}"`, async ({ request }) => {
                logger.banner(`SEO CACHE TEST — ${config.label}`);
                ...
                const spotChecks = getSpotCheckEndpoints('seo');
                await verifyNoUnrelatedKeysDeleted(request, 'seo', spotChecks);
                logger.pass('No unrelated keys affected');

                logger.banner('SEO CACHE TEST — PASSED ✅');
            });
