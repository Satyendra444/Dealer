...
import { AUTO_EXPO_TAGS } from '../../src/data/cache-tags';
import {
    warmCache,
...
test.describe('🎪 Auto-Expo — Cache Invalidation', () => {
    for (const config of AUTO_EXPO_TAGS) {
        test.describe(config.label, () => {

            test(`Full cache lifecycle for tag="${config.tag}"`, async ({ request }) => {
                logger.banner(`AUTO-EXPO CACHE TEST — ${config.label}`);
                ...
                const spotChecks = getSpotCheckEndpoints('auto-expo');
                await verifyNoUnrelatedKeysDeleted(request, 'auto-expo', spotChecks);
                logger.pass('No unrelated keys affected');

                logger.banner('AUTO-EXPO CACHE TEST — PASSED ✅');
            });
