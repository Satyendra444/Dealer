...
import { NAVIGATION_TAGS } from '../../src/data/cache-tags';
import {
    warmCache,
...
test.describe('🧭 Navigation — Cache Invalidation', () => {
    for (const config of NAVIGATION_TAGS) {
        test.describe(config.label, () => {

            test(`Full cache lifecycle for tag="${config.tag}"`, async ({ request }) => {
                logger.banner(`NAVIGATION CACHE TEST — ${config.label}`);
                ...
                const spotChecks = getSpotCheckEndpoints('navigation');
                await verifyNoUnrelatedKeysDeleted(request, 'navigation', spotChecks);
                logger.pass('No unrelated keys affected');

                logger.banner('NAVIGATION CACHE TEST — PASSED ✅');
            });
