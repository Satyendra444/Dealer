...
import { BRAND_TAGS } from '../../src/data/cache-tags';
import {
    warmCache,
...
test.describe('🏷️ Brand — Cache Invalidation', () => {
    for (const config of BRAND_TAGS) {
        test.describe(config.label, () => {

            test(`Full cache lifecycle for tag="${config.tag}"`, async ({ request }) => {
                logger.banner(`BRAND CACHE TEST — ${config.label}`);
                ...
                const spotChecks = getSpotCheckEndpoints('brand');
                await verifyNoUnrelatedKeysDeleted(request, 'brand', spotChecks);
                logger.pass('No unrelated keys affected');

                logger.banner('BRAND CACHE TEST — PASSED ✅');
            });
