...
import { CAREERS_TAGS } from '../../src/data/cache-tags';
import {
    warmCache,
...
test.describe('💼 Careers — Cache Invalidation', () => {
    for (const config of CAREERS_TAGS) {
        test.describe(config.label, () => {

            test(`Full cache lifecycle for tag="${config.tag}"`, async ({ request }) => {
                logger.banner(`CAREERS CACHE TEST — ${config.label}`);
                ...
                const spotChecks = getSpotCheckEndpoints('careers');
                await verifyNoUnrelatedKeysDeleted(request, 'careers', spotChecks);
                logger.pass('No unrelated keys affected');

                logger.banner('CAREERS CACHE TEST — PASSED ✅');
            });
