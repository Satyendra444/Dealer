...
import { SITE_HOME_TAGS } from '../../src/data/cache-tags';
import {
    warmCache,
...
test.describe('🏠 Site Home — Cache Invalidation', () => {
    for (const config of SITE_HOME_TAGS) {
        test.describe(config.label, () => {

            test(`Full cache lifecycle for tag="${config.tag}"`, async ({ request }) => {
                logger.banner(`SITE HOME CACHE TEST — ${config.label}`);
                ...
                const spotChecks = getSpotCheckEndpoints('site-home');
                await verifyNoUnrelatedKeysDeleted(request, 'site-home', spotChecks);
                logger.pass('No unrelated keys affected');

                logger.banner('SITE HOME CACHE TEST — PASSED ✅');
            });
