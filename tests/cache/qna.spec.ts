...
import { QNA_TAGS } from '../../src/data/cache-tags';
import {
    warmCache,
...
test.describe('💬 QnA — Cache Invalidation', () => {
    for (const config of QNA_TAGS) {
        test.describe(config.label, () => {

            test(`Full cache lifecycle for tag="${config.tag}"`, async ({ request }) => {
                logger.banner(`QNA CACHE TEST — ${config.label}`);
                ...
                const spotChecks = getSpotCheckEndpoints('qna');
                await verifyNoUnrelatedKeysDeleted(request, 'qna', spotChecks);
                logger.pass('No unrelated keys affected');

                logger.banner('QNA CACHE TEST — PASSED ✅');
            });
