...
import { RATING_TAGS } from '../../src/data/cache-tags';
import {
    warmCache,
...
test.describe('⭐ Rating — Cache Invalidation', () => {
    for (const config of RATING_TAGS) {
        test.describe(config.label, () => {

            test(`Full cache lifecycle for tag="${config.tag}"`, async ({ request }) => {
                logger.banner(`RATING CACHE TEST — ${config.label}`);
                ...
                const spotChecks = getSpotCheckEndpoints('rating');
                await verifyNoUnrelatedKeysDeleted(request, 'rating', spotChecks);
                logger.pass('No unrelated keys affected');

                logger.banner('RATING CACHE TEST — PASSED ✅');
            });
