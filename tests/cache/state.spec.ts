...
import { STATE_TAGS } from '../../src/data/cache-tags';
import {
    warmCache,
...
test.describe('📍 State — Cache Invalidation', () => {
    for (const config of STATE_TAGS) {
        test.describe(config.label, () => {

            test(`Full cache lifecycle for tag="${config.tag}"`, async ({ request }) => {
                logger.banner(`STATE CACHE TEST — ${config.label}`);
                ...
                const spotChecks = getSpotCheckEndpoints('state');
                await verifyNoUnrelatedKeysDeleted(request, 'state', spotChecks);
                logger.pass('No unrelated keys affected');

                logger.banner('STATE CACHE TEST — PASSED ✅');
            });
