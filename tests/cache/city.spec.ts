...
import { CITY_TAGS } from '../../src/data/cache-tags';
import {
    warmCache,
    ...
    for (const config of CITY_TAGS) {
        test.describe(config.label, () => {
            ...
            test(`Full cache lifecycle for tag="${config.tag}"`, async ({ request }) => {
                ...
                const spotChecks = getSpotCheckEndpoints('city');
                await verifyNoUnrelatedKeysDeleted(request, 'city', spotChecks);
                logger.pass('No unrelated keys affected');

                logger.banner('CITY CACHE TEST — PASSED ✅');
            });
